...
### 1.2.1 Class files
클래스의 이름은 [UpperCamelCase](http://en.wikipedia.org/wiki/CamelCase) 로 작성.  
Android 구성 요소를 확장하는 클래스의 경우 클래스 이름은 구성 요소 이름으로 끝나야 한다.   

EX) `SignInActivity`, `SignInFragment`, `ImageUploaderService`, `ChangePasswordDialog`

### 1.2.2 Resources files
리소스의 이름은 [lowercase_underscore]() 로 작성.
...
레이아웃 파일은 Android 구성 요소의 이름과 일치해야 하지만 최상위 구성 요소 이름은 처음으로 이동해야함.  
예를 들어, `SignInActivity.kt` -> `activity_sign_in.xml`
...
레이아웃 파일과 마찬가지로 메뉴 파일은 구성 요소의 이름과 일치해야 한다.  
예를 들어 `UserActivity` 에서 사용할 메뉴 파일의 이름은 `activity_user` 와 같이 정의한다.
이미 메뉴 디렉토리 안에 있기 때문에 `menu` 라는 단어를 포함하지 않는 것이 좋다.

#### 1.2.2.4 Values files

values 의 파일들은 복수 형태로 표시한다.
EX)`strings.xml`, `styles.xml`, `colors.xml`, `dimens.xml`, `attrs.xml`
